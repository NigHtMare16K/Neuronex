...
from config.settings import (
    ...
    SPIKE_SUBSAMPLE, MAX_SPIKES_FRAME,
)
...
class SpikeEncoder:
    ...
    def __init__(self, frame_buffer: FrameBuffer, event_queue: EventQueue,
                 on_spikes=None):
        self.frame_buffer  = frame_buffer
        self.event_queue   = event_queue
        self._on_spikes    = on_spikes
    ...
    def _encode_frame(self, frame: np.ndarray) -> list[SpikeEvent]:
        ...
        step = SPIKE_SUBSAMPLE
        on_mask  = delta[::step, ::step] >  SPIKE_THRESHOLD
        off_mask = delta[::step, ::step] < -SPIKE_THRESHOLD

        t_now = time.perf_counter() * 1000.0
        spikes: list[SpikeEvent] = []
        budget = MAX_SPIKES_FRAME

        ys, xs = np.where(on_mask)
        for y, x in zip(ys, xs):
            if len(spikes) >= budget:
                break
            spikes.append(SpikeEvent(
                x=int(x * step), y=int(y * step),
    ...
    def sparsity(self) -> float:
        """Fraction of pixels that did NOT spike — higher is better."""
